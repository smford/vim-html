...
- **Shell Commands**: `vim <file>`, `cat` (with rich **GitHub Flavored Markdown (GFM)** graphical rendering for `.md` files including alerts, tables, task lists, and syntax-highlighted code blocks, plus `--raw` flag), `curl` (with `-o`, `-O`, `-s`, `-I`, `-v`, `-X`, `-H`, `-d`), `wget`, `ls` (with `-l`, `-a`, `-la` and colorized file types), `cd`, `pwd`, `mkdir -p`, `touch`, `rm -r`, `cp`, `mv`, `echo` (with `>` and `>>` redirection), `clear`, `grep`, `top` / `htop` (live CPU/RAM bars and process lists), `ps aux`, `df -h`, `free -m`, `dmesg`, `whoami`, `uname -a`, `uptime`, `date`, `history`, `tree`, `download`, `upload`, `vfs-export`, `vfs-import`, `theme`, and `reboot`.
